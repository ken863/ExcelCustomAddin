...
namespace ExcelCustomAddin
{
  ...
  public static class UtilityService
  {
    ...
    /// 2. Tính số thứ tự dựa trên cột và starting number
    /// 3. Format số thứ tự theo cấu hình
    /// 4. Kết hợp prefix + số thứ tự
    ///
    /// Ví dụ: Column A, prefix "INV", starting number 1, format "D3"
    /// → Result: "INV001"
    /// </summary>
    ...
    public static string GenerateAutoSheetName(Worksheet activeSheet, int column, string currentSheetName)
    {
      try
      {
        ...
        // Lấy starting number từ cấu hình chung
        int startingNumber = SheetConfigManager.GetGeneralConfig()?.StartingNumber ?? 1;

        // Tính số thứ tự: column + startingNumber - 1
        // Column A = 1, nếu startingNumber = 1 thì A = 1
        // Column A = 1, nếu startingNumber = 100 thì A = 100
        int sequenceNumber = column + startingNumber - 1;
